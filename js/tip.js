...
// ask and wait
const send_money = async (amount, address) => {
...
select_form.addEventListener("submit", (event) => {
  ...
  if (window.ethereum) {
    ...
    if (input.value < 0) {
      ...
      send_money(input.value, destination_addr);
      alert("sent! thank you for tipping!!");
      select_form.classList.add("has_eth");
    }
  } else {
    alert(
      `you are not connected to web3 wallet or your browser doesn't allow this functionality`
    );
  }
});
